...
class ActorCritic(nn.Module):
    ...
    def pi(self, x, softmax_dim=0):
        ...
        logits = torch.clamp(x, min=-10000, max=10000)
    ...
    def train_net(self):
        ...
        pi_a = pi.gather(1, a)
        pi_a = torch.clamp(pi_a, min=1e-8, max = 1e+8)
...
class Agent():
    ...
    def train(self, model):
        ...
        for _ in range(self.epochs):
            ...
            # print(len(self.env.mem_action), len(self.env.mem_states))
            ...
            while not done:
                for t in range(self.n_rollout):
                    ...
                    model.put_data((s, ground_action, r, s_prime, done))
...
if __name__ == "__main__":
    ...
    for d in datasets:
        ...
        for i, test_user_log in enumerate((user_list)):
            ...
            trained_ac_model, best_lr, best_gamma, training_accuracy = training(train_files, env, d, 'Actor-Critic', 5)
            X_train.append(training_accuracy)
            # test user
            test_files = [test_user_log]
            testing_accu, gp = testing(test_files, env, trained_ac_model, best_lr, best_gamma, d, 'Actor-Critic')
