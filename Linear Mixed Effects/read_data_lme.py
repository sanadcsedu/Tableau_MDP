...
class read_data_lme:
    ...
    def read_cur_data(self, userid, dataset, task):
        ...
        query = 'SELECT state, timestamp FROM master_file where dataset = ' + '\'' + str(dataset) + '\'' + ' and userid = ' + str(userid) + ' and task = \'' + str(task) + '\'' + ' order by seqId'
        # print(query)
        c.execute(query)
        cur_data = c.fetchall()
        # print(cur_data)
    ...
    def merge2(self, dataset, user):
        ...
        for t in self.tasks:
            ...
            for itrs in data:
                ...
                timestamps.append(itrs[1])
        return interactions, timestamps

    def merge_v2(self, user, dataset, task):
        interactions = []
        timestamps = []
        data = self.read_cur_data(user, dataset, task)
        # pdb.set_trace()
        for itrs in data:
            interactions.append(itrs[0])
            timestamps.append(itrs[1])
